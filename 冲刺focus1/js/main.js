...
//choose car star
	...
	for(var i=0;i<3;i++){
		car=$("<div></div>").addClass("car").css("background-image","url(images/car"+(i+1)+".png)").css("left",539*(i>0?1:0)).attr("state",carstate[i]);
			...
			this.run=function(){
				...
				if(turnCount>=80){
					if(!count){
						...
						for(var i=0;i<cars.length;i++){
							if(cars[i].x==channelX[startChannel+fx]){
								fx=0;
							}
						}
						count++;
...
/*					if(this.x>car.x){
		...
		this.start=function(carno){
			...
			div.css({"background":"url(images/movecar"+(carno+1)+".png) no-repeat center"});
			bg.append(div);
			for(var i=0;i<cars.length;i++){
				carIDs.push("other"+i);
...
//Game class end
	...
	function gameend(metre,totletime){
		$("#overtips").text("您在"+totletime/1000+"秒内冲刺了"+metre+"米");
		$("#promptp").show();
		$("#startprompt").hide();
		$("#endprompt").show();
	}
